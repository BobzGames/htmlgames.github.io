// (c) Robert Kleffner's
// Excanvas + Touch additions by Paul
...
Mario.BackgroundGenerator.prototype = {
    ...
    GenerateUnderground: function (a) {
        var b = 0,
            ...
            e = 0,
            d = 0;
        if (this.Distant) for (var f = 0, b = 0; b < this.Width; b++) {
    ...
    GenerateCastle: function (a) {
        var b = 0,
            ...
            e = 0,
            d = 0;
        if (this.Distant) for (b = 0; b < this.Width; b++) for (c = 0; c < this.Height; c++) e = b % 2, d = c - 1, d > 2 && d < 5 ? d = 2 : d >= 5 && (d -= 2), d < 0 ? (e = 0, d = 5) : d > 4 ? (e = 1, d = 5) : e < 1 && d === 3 ? (e = 0, d = 3) : e < 1 && d > 0 && d < 3 && (e = 0, d = 2), a.SetBlock(b, c, 1 + e + (d + 4) * 8);
        else for (b = 0; b < this.Width; b++) for (c = 0; c < this.Height; c++) e = b % 3, d = c - 1, d > 2 && d < 5 ? d = 2 : d >= 5 && (d -= 2), d < 0 ? (e = 1, d = 5) : d > 4 ? (e = 2, d = 5) : e < 2 && d === 4 ? (e = 2, d = 4) : e < 2 && d > 0 && d < 4 && (e = 4, d = -3), a.SetBlock(b, c, 1 + e + (d + 3) * 8)
    }
};
...
Mario.Character.prototype.SubMove = function (a, b) {
    ...
    if (b < 0) if (this.IsBlocking(this.X + a, this.Y + b - this.Height, a, b)) c = !0;
...
Mario.Character.prototype.Stomp = function (a) {
    ...
    else if (a instanceof Mario.Shell) Enjine.KeyboardInput.IsKeyDown(Enjine.Keys.A) && a.Facing === 0 ? (this.Carried = a, a.Carried = !0) : (Enjine.Resources.PlaySound("kick"), this.XJumpSpeed = 0, this.YJumpSpeed = -1.9, this.JumpTime = 8, this.Ya = this.JumpTime * this.YJumpSpeed, this.Sliding = this.OnGround = !1, this.InvulnerableTime = 1)
};
...
Mario.LevelGenerator.prototype = {
    ...
    BuildJump: function (a, b) {
        for (var c = (Math.random() * 4 | 0) + 2, e = c * 2 + ((Math.random() * 2 | 0) + 2), d = 0, f = 0, g = (Math.random() * 3 | 0) === 0, h = this.Height - 1 - (Math.random() * 4 | 0), d = b; d < b + e; d++) if (d < b + c || d > b + e - c - 1) for (f = 0; f < this.Height; f++) f >= h ? a.SetBlock(d, f, 145) : g && (d < b + c ? f >= h - (d - b) + 1 && a.SetBlock(d, f, 9) : f >= h - (b + e - d) + 2 && a.SetBlock(d, f, 9));
        return e
    },
    ...
    BuildHillStraight: function (a, b, c) {
        ...
        for (this.AddEnemyLine(a, b + 1, b + e - 1, d - 1); i;) if (h = h - 2 - Math.random() * 3 | 0, h <= 0) i = !1;
    ...
    AddEnemyLine: function (a, b, c, e) {
        for (var d = 0, f = 0, d = b; d < c; d++) if ((Math.random() * 35 | 0) < this.Difficulty + 1) f = Math.random() * 4 | 0, this.Difficulty < 1 ? f = Mario.Enemy.Goomba : this.Difficulty < 3 && (f = Math.random() * 3 | 0), a.SetSpriteTemplate(d, e, new Mario.SpriteTemplate(f, (Math.random() * 35 | 0) < this.Difficulty))
    },
    BuildTubes: function (a, b, c) {
        ...
        for (h = b; h < b + e; h++) {
            ...
            for (i = 0; i < this.Height; i++) if (i >= d) a.SetBlock(h, i, 145);
...
Mario.Enemy.prototype.CollideCheck = function () {
    if (this.DeadTime === 0) {
        ...
        if (a > -this.Width * 2 - 4 && a < this.Width * 2 + 4 && b > -this.Height && b < Mario.MarioCharacter.Height) if (this.Type !== Mario.Enemy.Spiky && Mario.MarioCharacter.Ya > 0 && b <= 0 && (!Mario.MarioCharacter.OnGround || !Mario.MarioCharacter.WasOnGround)) if (Mario.MarioCharacter.Stomp(this), this.Winged) this.Winged = !1, this.Ya = 0;
        else {
            this.YPicO = 7;
            this.PicHeight = 8;
...
Mario.FlowerEnemy.prototype.Move = function () {
    ...
    if (this.DeadTime > 0) {
        ...
        this.Ya *= 0.95;
        this.Ya += 1
    } else this.Tick++, this.Y >= this.YStart ? (this.YStart = this.Y, a = Math.abs(Mario.MarioCharacter.X - this.X) | 0, this.JumpTime++, this.Ya = this.JumpTime > 40 && a > 24 ? -8 : 0) : this.JumpTime = 0, this.Y += this.Ya, this.Ya *= 0.9, this.Ya += 0.1, this.XPic = ((this.Tick / 2 | 0) & 1) * 2 + ((this.Tick / 6 | 0) & 1)
};
...
Mario.LoadingState.prototype.Enter = function () {
    ...
    this.Images[0].src = "images/bgsheet.gif"; //png
    this.Images[1].src = "images/endscene.gif";
    this.Images[2].src = "images/enemysheet.gif"; //png
    this.Images[3].src = "images/firemariosheet.gif"; //png
    this.Images[4].src = "images/font.gif";
    this.Images[5].src = "images/gameovergost.gif";
    this.Images[6].src = "images/itemsheet.gif"; //png
    this.Images[7].src = "images/logo.gif";
    this.Images[8].src = "images/mapsheet.gif"; //png
    this.Images[9].src = "images/mariosheet.gif"; //png
    this.Images[10].src = "images/particlesheet.gif"; //png
    this.Images[11].src = "images/racoonmariosheet.gif"; //png
    this.Images[12].src = "images/smallmariosheet.gif"; //png
    this.Images[13].src = "images/title.gif";
    this.Images[14].src = "images/worldmap.gif"; //png
    Enjine.Resources.AddImages(this.Images);
	if(!isIE){
		if(bTouch){ // pf
			Enjine.Resources.AddSound("1up", "sounds/blank.wav", 1).AddSound("breakblock", "sounds/blank.wav").AddSound("bump", "sounds/blank.wav", 2).AddSound("cannon", "sounds/blank.wav").AddSound("coin", "sounds/blank.wav", 5).AddSound("death", "sounds/blank.wav", 1).AddSound("exit", "sounds/blank.wav", 1).AddSound("fireball", "sounds/blank.wav", 1).AddSound("jump", "sounds/blank.wav", 1).AddSound("kick", "sounds/blank.wav", 1).AddSound("message", "sounds/blank.wav", 1).AddSound("pipe", "sounds/blank.wav", 1).AddSound("powerdown", "sounds/blank.wav", 1).AddSound("powerup", "sounds/blank.wav", 1).AddSound("sprout", "sounds/blank.wav", 1).AddSound("stagestart", "sounds/blank.wav", 1).AddSound("stomp", "sounds/blank.wav", 1);
		}else{
...
Mario.MapState = function () {
    ...
    if(isIE){ // ie IE
		G_vmlCanvasManager.initElement(this.MapImage);
	}  
    ...
    this.WorldNumber = -1;
    try{if(window.location.search.split("?")[1])this.WorldNumber=parseInt(window.location.search.split("?")[1],10)-2}catch(e){}
    this.NextWorld()
};
...
Mario.MapState.prototype.GenerateLevel = function () {
    ...
    for (a = 0; a < 21; a++) for (b = 0; b < 16; b++) if (this.Level[a][b] === Mario.MapTile.Grass && (a !== this.XFarthestCap || b !== this.YFarthestCap - 1)) if (c = g.PerlinNoise(a * 10 + h, b * 10 + i), c > 0) this.Level[a][b] = Mario.MapTile.Decoration;
    return !0
};
...
Mario.MapState.prototype.Travel = function (a, b, c, e) {
    if (!(this.Level[a][b] !== Mario.MapTile.Road && this.Level[a][b] !== Mario.MapTile.Level)) {
        ...
        if (this.Level[a][b] === Mario.MapTile.Level) if (this.Data[a][b] > 0) this.Data[a][b] = this.LevelId !== 0 && (Math.random() * 4 | 0) === 0 ? -3 : ++this.LevelId;
...
Mario.MapState.prototype.Update = function (a) {
    ...
    if (this.WorldNumber !== 8) {
        ...
        Mario.MarioCharacter.Large ? (this.LargeMario.X = this.XMario + this.XMarioA * a | 0, this.LargeMario.Y = this.YMario + (this.YMarioA * a | 0) - 22, this.LargeMario.Update(a)) : (this.SmallMario.X = this.XMario + this.XMarioA * a | 0, this.SmallMario.Y = this.YMario + (this.YMarioA * a | 0) - 6, this.SmallMario.Update(a))
    }
};
...
Mario.LevelState.prototype.Update = function (a) {
    ...
    for (b = this.FireballsOnScreen = 0; b < this.Sprites.Objects.length; b++) d = this.Sprites.Objects[b], d !== Mario.MarioCharacter && (c = d.X - this.Camera.X, e = d.Y - this.Camera.Y, c < -64 || c > 384 || e < -64 || e > 304 ? this.Sprites.RemoveAt(b) : d instanceof Mario.Fireball && this.FireballsOnScreen++);
    if (this.Paused) for (b = 0; b < this.Sprites.Objects.length; b++) this.Sprites.Objects[b] === Mario.MarioCharacter ? this.Sprites.Objects[b].Update(a) : this.Sprites.Objects[b].UpdateNoMove(a);
        ...
        b = !1;
        this.Tick++;
        for (c = (this.Camera.X / 16 | 0) - 1; c <= ((this.Camera.X + this.Layer.Width) / 16 | 0) + 1; c++) for (e = (this.Camera.Y / 16 | 0) - 1; e <= ((this.Camera.Y + this.Layer.Height) / 16 | 0) + 1; e++) {
            d = 0;
