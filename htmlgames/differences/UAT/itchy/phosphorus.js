  ...
  Base.prototype.effects = function(costume, isStage) {
	...
	var cih = (isStage) ? 360 : costume.image.height;
	  ...
	  var effect = effectsContext.getImageData(0, 0, ciw, cih);
	    ...
	    // cheap 'n' nasty way...
            if (effect.data[i + 0] > 0 ) effect.data[i + 0] = (effect.data[i + 0] + colorVal) & 0xff;
            if (effect.data[i + 1] > 0 ) effect.data[i + 1] = (effect.data[i + 1] + colorVal) & 0xff;
            if (effect.data[i + 2] > 0 ) effect.data[i + 2] = (effect.data[i + 2] + colorVal) & 0xff;
            if (effect.data[i + 3] > 0 ) effect.data[i + 3] = effect.data[i + 3]; // alpha
