	...
	init: function() {
		...
		var draw = function(e) {
			...
			if(e.type === "canvas") {
				...
				if(_pf){
					Crafty.background("url('images/splat.jpg') no-repeat");
					setTimeout("_pf=false;Crafty.background(\"url('images/bg.jpg') no-repeat\");",400);
				}else{
					this._element.style.background = "url('" + this.__image + "') no-repeat -" + co.x + "px -" + co.y + "px";	
				}
				if(bTouch){
					this._element.ontouchstart=function(){scoreEnt.text("Score: POW!");score += 255;_pf=true;}; // pf
