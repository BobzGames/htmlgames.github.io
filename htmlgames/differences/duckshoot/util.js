...
function makeUnfave(id) {
    var uid = window.localStorage["mas-uid"];
...
function util_createUnknownAccount() {
    if (window.localStorage["mas-uid"] == null) {
        $.ajax({
            type: "POST",
            url: "Magic.ashx",
            data: "c=lg",
            success: function (xml) {
                if (isAJAXSuccess(xml)) {
                    var uid = $(xml).find("uid").text();
                    var up = $(xml).find("upic").text();
                    var id = $(xml).find("id").text();
                    var un = $(xml).find("un").text();

                    util_setUserToken(uid, id, 2, un, up);
                }
            }
        });
    }
