...
function VectorlightGame() {
    ...
    this.gameInit = function (a, b, c, d, e) {
        this.gameInit2(a, b, c, d, e, !false) // webkit fix (after THEY broke it!)
    };
    this.gameInit2 = function (a, b, c, d, e, f) {
        ...
        if (this.userID == "") {
            this.userIDDisplay = "http://goo.gl/zI6A"
        }
        this.setUrl();
    ...
    this.setUrl = function () {
        ...
        if (window.location.hostname == "localhost") {
            //this.baseURL = "http://localhost/mas/"
        } else {
            //this.baseURL = "http://www.magicappstore.com/"
        }
    ...
    this.recordGamePlay = function () {
        try {
            var a = this.deviceName;
            var b = this.serviceURL + "?rg=1&gid=" + this.pageID + "&u=" + encodeURIComponent(this.userID) + "&i=" + encodeURIComponent(a + "-" + this.instanceID);
            $.get(b, function (a, b) {}, "html")
        } catch (c) {}
    };
    ...
    this.saveAchievement = function (a, b) {
        ...
        if (c != null) {
            var e = Math.floor(b * c.length * 5 / 8);
            $.ajax({
                type: "POST",
                url: this.masUrl,
                data: "c=b&uid=" + c + "&tid=" + a + "&v=" + b + "&k=" + e + "&un=" + encodeURIComponent(d)
            })
        }
    };
    this.getUserId = function () {
    	if(navigator.userAgent.match("MSIE"))return "http://goo.gl/zI6A"
        return window.localStorage["mas-uid"]
    };
    this.getUsername = function () {
    	if(navigator.userAgent.match("MSIE"))return "http://goo.gl/zI6A"
        return window.localStorage["mas-un"]
    };
    this.setUsername = function (a) {
    	if(navigator.userAgent.match("MSIE"))return
        window.localStorage["mas-un"] = a
    };
    ...
    this.getBestAchievement = function (a, b, c) {
        $.ajax({
            ...
            data: "c=z&tid=" + a,
            success: function (a) {
                var d = $(a).find("un").text();
                var e = $(a).find("sc").text();
                if (b != null) {
